...
def build_rounds_for_day(calls: List[Call], day: str, clients_by_id, locked_starts: dict = None) -> tuple:
    """
    Returns (rounds, unplaced_errors, skipped_patterns) for a single day.
    `locked_starts` (call_id -> scheduled_start_minutes) forces recurring
    calls to keep the exact time they were given on the anchor day.
    """
    locked_starts = locked_starts or {}
    skipped_patterns = []
    ...
    while unassigned:
        ...
        while True:
            ...
            for idx, candidate in plausible_candidates:
                ...
                if is_locked:
                    # Locked calls cannot shift at all — the earliest we
                    # could arrive must be at/before the locked time.
                    ...
                    if earliest_possible_start > new_start:
                        continue  # can't get there in time without shifting — skip
                else:
                    earliest_possible_start = last.scheduled_end_minutes + travel
    ...
    return rounds, unplaced_errors, skipped_patterns
...
if __name__ == "__main__":
    ...
    locked_starts = {}  # call_id -> scheduled_start_minutes, set after the anchor day
    ...
    for day_index, day in enumerate(days_to_process):
        ...
        # After the FIRST day processed, lock every recurring call's time
        # so it stays identical on every later day it occurs.
        if day_index == 0:
